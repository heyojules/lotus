...
func (m *DashboardModel) formatLogDetails(entry model.LogRecord, maxWidth int) string {
	// Define styles
	...
	normalizedSeverity := normalizeSeverityLevel(entry.Level)
	...
	details.WriteString(labelStyle.Render("Severity:") + " " +
		severityStyle.Render(entry.Level) + "\n")
...
func (m *DashboardModel) formatAttributeValuesModal(entry *AttributeEntry, maxWidth int) string {
