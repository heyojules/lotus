...
backup-enabled: true
backup-interval: 1h
backup-local-dir: /tmp/tiny-telemetry-backups
backup-keep-last: 10
backup-bucket-url: s3://my-bucket/tiny-telemetry
...
`,
			assert: func(t *testing.T, cfg appConfig) {
				...
				if cfg.BackupBucketURL != "s3://my-bucket/tiny-telemetry" {
...
func resetTinyTelemetryEnv(t *testing.T) {
	t.Helper()
	...
	for _, kv := range os.Environ() {
		...
		if !ok || !strings.HasPrefix(key, "TINY_TELEMETRY_") {
			continue
		}
		original[key] = value
		existed[key] = true
